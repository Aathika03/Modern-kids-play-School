// src/pages/gallery.tsx
import Head from "next/head";
import { useState } from "react";

export default function Gallery() {
  const images = [
    { src: "/gallery/activity.jpg", caption: "Creative Playtime Moments" },
    { src: "/gallery/activity2.jpg", caption: "Hands-On Learning Activities" },
    { src: "/gallery/classroom.jpeg", caption: "Our Colorful Classroom" },
    { src: "/gallery/activity11.jpg", caption: "Learning maths with Blocks" },
    { src: "/gallery/activity3.jpg", caption: "Joyful Group Activities" },
    { src: "/gallery/activity15.jpg", caption: "Creative Learning" },
    { src: "/gallery/activity5.jpg", caption: "Art & Imagination at Work" },
    { src: "/gallery/activity6.jpeg", caption: "Fun with Early Learning Tools" },
    { src: "/gallery/activity7.jpeg", caption: "Exploring Through Play" },
    { src: "/gallery/toddlers.jpg", caption: "Exploring Different Tools" },
    { src: "/gallery/activity8.jpeg", caption: "Smiles That Brighten Our Day" },
    { src: "/gallery/toddlers1.jpg", caption: "Best Time Together" },
    { src: "/gallery/activity9.jpeg", caption: "Outdoor Fun and Movement" },
    { src: "/gallery/activity4.jpeg", caption: "Creative Expressions" },
    { src: "/gallery/activity1.jpeg", caption: "Creative Painting" },
    { src: "/gallery/activity10.jpeg", caption: "Learning with Curiosity" },
    { src: "/gallery/playground.jpg", caption: "Playground Adventures" },
    { src: "/gallery/activity12.jpeg", caption: "Creative Sketching" },
    { src: "/gallery/activity14.jpg", caption: "Early Maths" },
    { src: "/gallery/activity13.jpg", caption: "Building Blocks" },
  ];

  const [lightboxOpen, setLightboxOpen] = useState(false);
  const [currentImage, setCurrentImage] = useState<{ src: string; caption: string } | null>(null);

  const openLightbox = (img: { src: string; caption: string }) => {
    setCurrentImage(img);
    setLightboxOpen(true);
  };

  const closeLightbox = () => {
    setLightboxOpen(false);
    setCurrentImage(null);
  };

  return (
    <>
      <Head>
        <title>Gallery — Little Flower Play School</title>
      </Head>

      <main className="min-h-screen bg-[#c6e2eeff] py-10 px-4 sm:px-6 md:px-10">

        {/* INTRO SECTION */}
        <section className="max-w-5xl mx-auto text-center mb-14 px-2">
          <h1 className="text-3xl sm:text-4xl font-extrabold text-[#204d63] mb-6">
            A Glimpse Into Our Little World
          </h1>

          <p className="text-base sm:text-lg text-gray-700 leading-relaxed max-w-3xl mx-auto">
            At Little Flower Play School, every day is a new beginning filled with discovery,
            laughter, and growth. Our children learn not only through books, but through hands-on
            experiences that spark imagination and build confidence.
            <br /><br />
            Our gallery captures the heart of our school — little hands creating, little feet
            exploring, and little minds blooming with ideas.
          </p>
        </section>

        {/* PAGE TITLE */}
        <div className="max-w-5xl mx-auto text-center mb-10">
          <h2 className="text-2xl sm:text-3xl font-bold text-[#333]">Our Gallery</h2>
          <p className="text-gray-600 mt-2 sm:text-lg">
            Tap an image to view it in full size!
          </p>
        </div>

        {/* GALLERY GRID */}
        <div className="
          max-w-6xl mx-auto 
          grid 
          grid-cols-2 
          sm:grid-cols-3 
          md:grid-cols-3 
          lg:grid-cols-4 
          gap-4 sm:gap-6 md:gap-8
        ">
          {images.map((img, idx) => (
            <div
              key={idx}
              className="bg-white rounded-xl shadow-md overflow-hidden cursor-pointer 
                         hover:scale-[1.04] transition-transform duration-300"
              onClick={() => openLightbox(img)}
            >
              <img
                src={img.src}
                alt={img.caption}
                className="w-full h-32 sm:h-40 md:h-48 lg:h-52 object-cover"
                onError={(e) => {
                  (e.target as HTMLImageElement).src = "/placeholder.jpg";
                }}
              />
              <p className="text-center text-xs sm:text-sm font-medium text-gray-700 py-2 px-2">
                {img.caption}
              </p>
            </div>
          ))}
        </div>

        <div className="h-20" />
      </main>

      {/* LIGHTBOX OVERLAY */}
      {lightboxOpen && currentImage && (
        <div
          className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-[9999] px-4"
          onClick={closeLightbox}
        >
          <div className="max-w-3xl w-full mx-auto p-2 sm:p-4">
            <img
              src={currentImage.src}
              alt={currentImage.caption}
              className="max-h-[80vh] w-full object-contain rounded-lg shadow-lg"
            />
            <p className="text-center text-white mt-4 text-base sm:text-lg">
              {currentImage.caption}
            </p>
          </div>
        </div>
      )}
    </>
  );
}
